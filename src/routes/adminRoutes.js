...
adminRoutes.get('/otps', validate(z.object({
  query: z.object({
    type: z.enum(['service', 'login']).optional(),
  ...
  const result = await query(
    `with all_otps as (
       ...
       select
         ...
         lo.code,
         1 as max_uses,
         case when lo.status = 'used' then 1 else 0 end as used_count,
         lo.expires_at,
         lo.status,
         lo.created_at
       from login_otps lo
     ...
     select
       coalesce(json_agg(filtered order by created_at desc) filter (where id is not null), '[]') as items,
    ...
    [type, status, shopId, pageSize, offset]
  );
